...
# ----------------- Данные: алгоритмы -----------------
# Для каждого алгоритма указан: name, category, summary, rules, explanation, pseudocode, example, result, media (URL)
ALGORITHMS: Dict[str, Dict] = {
    ...
    # ПОИСК (добавлены 2 новых: exponential_search, interpolation_search)
    "binary_search": {
        "name": "🔍 Бинарный поиск",
        "category": "Поиск",
    ...
    # ДИНАМИЧЕСКОЕ ПРОГРАММИРОВАНИЕ (добавлены LCS и Levenshtein)
    "knapsack": {
    ...
    "example_lcs": {
        ...
        "category": "work_examples",
        "summary": "Введите две строки → бот покажет DP-таблицу и длину LCS.",
...
# ----------------- Упрощенные интерактивные примеры -----------------
INTERACTIVE_ALGORITHMS = {
    ...
    "interactive_selection_sort": {
        "name": "🔧 Сортировка выбором",
        "category": "interactive_demo",
        "prompt": "Введите массив чисел через пробел.\nПример: 64 25 12 22 11",
    },
    "interactive_insertion_sort": {
        "name": "🔧 Сортировка вставками",
        "category": "interactive_demo",
        "prompt": "Введите массив чисел через пробел.\nПример: 12 11 13 5 6",
    },
    "interactive_linear_search": {
        "name": "🔧 Линейный поиск",
        "category": "interactive_demo",
...
def sorted_categories():
    ...
    # исключаем интерактивную категорию полностью, чтобы она не попала в меню
    categories = {v["category"] for v in ALGORITHMS.values()}
    if "interactive_demo" in categories:
        categories.remove("interactive_demo")
    return sorted(categories)

def algs_in_category(category: str):
    items = [(k, v["name"]) for k, v in ALGORITHMS.items() if v["category"] == category]
...
def format_algorithm_text(alg: Dict) -> str:
    parts = [
        f"{alg['name']}",
        "",
        f"💡 Суть:\n{alg['summary']}",
...
# Безопасный лимит (Telegram ~ 4096)
MAX_TEXT_LEN = 3800

# ----------------- Клавиатуры -----------------
def make_main_menu(columns: int = 2) -> types.InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()

    # Основные категории (показываем имя категории как есть)
...
def make_category_menu(category: str, columns: int = 3) -> types.InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for key in algs_in_category(category):
        builder.button(text=ALGORITHMS[key]["name"], callback_data=f"alg_{key}")
    # служебные кнопки
...
def make_back_menu() -> types.InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
def make_interactive_menu() -> types.InlineKeyboardMarkup:
    """Меню интерактивных демонстраций"""
    builder = InlineKeyboardBuilder()
    for key, data in INTERACTIVE_ALGORITHMS.items():
...
# ----------------- Отправка медиа -----------------
async def send_media(chat_id: int, media_url: str) -> None:
    """
    ...
    try:
        ...
        if low.endswith((".gif", ".mp4", ".webm")):
            # animation
            await bot.send_animation(chat_id, animation=media_url)
        else:
            # photo / svg as png (telegram supports direct svg? better send as photo; some svg may not load)
            await bot.send_photo(chat_id, photo=media_url)
    except Exception:
        # молча падаем — не критично, но логируем в консоль
        print(f"[WARN] Не удалось отправить media: {media_url}")

# ----------------- Обработчики -----------------
@dp.message(Command(commands=["start"]))
async def cmd_start(message: types.Message):
    text = "👋 Привет! Это бот-справочник по алгоритмам.\nВыбери категорию:"
    await message.answer(text, reply_markup=make_main_menu())
...
async def callbacks_handler(callback: CallbackQuery):
    ...
    if data.startswith("cat_"):
        ...
        await callback.message.edit_text(
            ...
            reply_markup=make_category_menu(category)
        )
    ...
    if data.startswith("random_"):
        ...
        candidates = algs_in_category(category)
        if not candidates:
    ...
    if data.startswith("alg_"):
        ...
        media_url = alg.get("media", "")
        if media_url:
            await send_media(chat_id, media_url)

        # 2) Формируем текст и отправляем
        full_text = format_algorithm_text(alg)
        if len(full_text) <= MAX_TEXT_LEN:
            await bot.send_message(chat_id, full_text, reply_markup=make_back_menu())
        else:
            # аккуратно разбиваем на части
            chunks = []
            cur = ""
            for line in full_text.splitlines(keepends=True):
                if len(cur) + len(line) > MAX_TEXT_LEN:
                    chunks.append(cur)
                    cur = line
                else:
                    cur += line
            ...
            for i, chunk in enumerate(chunks):
                if i < len(chunks) - 1:
                    ...
                    await bot.send_message(chat_id, chunk, reply_markup=make_back_menu())
    ...
    if data == "interactive_menu":
        await callback.message.edit_text(
            ...
            reply_markup=make_interactive_menu(),
            parse_mode="Markdown"
        )
        await callback.answer()
        return
    ...
    if data.startswith("inter_"):
        key = data[len("inter_"):]
        info = INTERACTIVE_ALGORITHMS.get(key)
...
@dp.message()
async def handle_interactive_input(message: types.Message):
    ...
    # ----------------- БИНАРНЫЙ ПОИСК -----------------
    if key == "interactive_binary_search":
        try:
            parts = text.split()
            if len(parts) < 2:
                await message.answer("Введите массив и число для поиска через пробел.\nПример: 1 2 3 4 5 4")
                return
            ...
            if arr != sorted(arr):
                await message.answer("⚠️ Массив должен быть отсортирован по возрастанию!")
                return
            ...
            result = "\n".join(steps)
            await message.answer(f"📘 Ход работы бинарного поиска:\n\n{result}")
    ...
    if key == "interactive_bubble_sort":
        try:
            ...
            await message.answer("📘 Ход работы пузырьковой сортировки:\n" + "\n".join(steps))
        except:
            await message.answer("Ошибка формата. Введите числа через пробел.\nПример: 5 2 8 1 3")
        dp.key = None
        return

    # ----------------- СОРТИРОВКА ВЫБОРОМ -----------------
    if key == "interactive_selection_sort":
        try:
            arr = list(map(int, text.split()))
            steps = []
            a = arr[:]  # копия массива для сортировки
            ...
            steps.append(f"\n✅ Итоговый отсортированный массив: {a}")
            await message.answer("📘 Ход работы сортировки выбором:\n" + "\n".join(steps))
        except:
            await message.answer("Ошибка формата. Введите числа через пробел.\nПример: 64 25 12 22 11")
        dp.key = None
        return
    ...
    if key == "interactive_insertion_sort":
        try:
            arr = list(map(int, text.split()))
            steps = []
            a = arr[:]  # копия массива для сортировки
            ...
            steps.append(f"\n✅ Итоговый отсортированный массив: {a}")
            await message.answer("📘 Ход работы сортировки вставками:\n" + "\n".join(steps))
        except:
            await message.answer("Ошибка формата. Введите числа через пробел.\nПример: 12 11 13 5 6")
        dp.key = None
        return
    ...
    # ----------------- ЛИНЕЙНЫЙ ПОИСК -----------------
    if key == "interactive_linear_search":
        try:
            parts = text.split()
            if len(parts) < 2:
                await message.answer("Введите массив и число для поиска через пробел.\nПример: 5 2 8 1 3 8")
            ...
            result = "\n".join(steps)
            await message.answer(f"📘 Ход работы линейного поиска:\n\n{result}")
        except:
            await message.answer("Ошибка формата. Введите массив и число через пробел.\nПример: 5 2 8 1 3 8")
        dp.key = None
        return
